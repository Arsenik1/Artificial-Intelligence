...
def inv_pendulum_train(num_of_episodes):
	...
	num_of_weights = 4
	population_size = 500
	w = [[0 for _ in range(num_of_weights)] for _ in range(population_size)]
    
	for i in range(population_size):
		for j in range(num_of_weights):
			w[i][j] = np.random.randint(-1.5, 1.5)
	...
	for episode in range(num_of_episodes):
		...
		if episode % ((num_of_episodes - 1) / 2) == 0:
			...
			best.write(str(best_solutions[0]))
			best.close()
			print("\n----------------------------------\n")
...
inv_pendulum_train(num_of_episodes = 100)
